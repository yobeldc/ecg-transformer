"""
ECG Digitization — Phone Camera or PNG (Streamlit)
--------------------------------------------------
Run locally:
...
Open from your phone:
  - Ensure your PC and phone are on the same network, then open:
      http://<YOUR_PC_IP>:8501
  - Or deploy to Streamlit Community Cloud / your HTTPS host.

Requirements (pin to avoid NumPy 2 ABI issues for some wheels):
  streamlit>=1.25
  numpy>=1.24,<2.0
  opencv-python
  torch
...
# Page / Sidebar
# ============================================================
st.set_page_config(page_title="ECG Digitization — Phone Camera / PNG", layout="wide")
st.title("ECG Digitization (PNG or Phone Camera → trace)")
st.caption("Capture with your phone camera or upload PNG → normalize (warp, deskew, resize) → detect boxes → digitize traces → overlay & metrics.")
...
with st.sidebar:
    ...
    # Discover signal checkpoints automatically
    ...
    MODEL_TYPE = st.selectbox(
        ...
        index=1,
        help="Matches your ColumnSignalTransformer (vanilla), DyT, or the TCN alternative."
    )

    st.markdown("---")
    st.subheader("Signal params")
    ...
    MAX_LEN = st.number_input("MAX_LEN (pos enc len)",  min_value=512, max_value=8192, value=4096, step=256)
    IMG_SIZE_BBOX = st.number_input("BBox img size",     min_value=128, max_value=512, value=256, step=16)

    st.markdown("---")
    st.subheader("Camera → standardize")
    TARGET_WIDTH = st.number_input("Output width (px)", min_value=1200, max_value=4000, value=2200, step=50,
                                   help="Captured photos are warped/deskewed then resized to this width.")
    DO_WARP   = st.toggle("Auto page warp (largest 4-corner sheet)", value=True)
    DO_DESKEW = st.toggle("Auto deskew (Hough-based)", value=True)
    MANUAL_ROT = st.slider("Manual rotate (°)", min_value=-30, max_value=30, value=0,
                           help="Applied after auto. Keep within ±30° to match your prior constraints.")
    CONTRAST = st.toggle("Mild contrast/denoise for camera shots", value=True)

# ============================================================
# Constants / Colors
# ============================================================
LEAD_NAMES = ["I","II","III","aVR","aVL","aVF","V1","V2","V3","V4","V5","V6","II_bottom"]
GT_COLOR   = (0, 255, 255)  # cyan (BGR)
PRED_COLOR = (255, 255, 0)  # yellow
BOX_COLOR  = (0, 255, 255)  # cyan

# ============================================================
# Utility: robust image IO / conversions
# ============================================================
def read_image_from_bytes(buf: bytes) -> np.ndarray:
...
# ============================================================
# Standardization pipeline for camera images
#   1) Optional page warp (largest 4-corner contour)
#   2) Auto deskew via Hough lines
#   3) Manual rotation (±30°)
#   4) Resize to TARGET_WIDTH
#   5) Mild denoise/contrast if enabled
# ============================================================
def _largest_quad_contour(bgr: np.ndarray) -> np.ndarray | None:
...
def _warp_from_quad(bgr: np.ndarray, quad: np.ndarray) -> np.ndarray:
    q = _order_quad(quad)
    (tl, tr, br, bl) = q
...
def _auto_skew_angle(bgr: np.ndarray) -> float:
    # Estimate page skew angle (in degrees) using Hough transform
    gray = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)
    gray = cv2.GaussianBlur(gray, (5,5), 0)
    ...
    for r_theta in lines[:200]:
        ...
        # map near 0/180 lines to 0 deg baseline
        if ang > 90: ang -= 180
        if -45 <= ang <= 45:
            angles.append(ang)
    if not angles:
...
def _rotate_bound(bgr: np.ndarray, angle_deg: float) -> np.ndarray:
...
def standardize_camera_rgb(rgb_in: np.ndarray,
                           warp: bool,
                           deskew: bool,
                           manual_rot_deg: int,
                           target_width: int,
                           enhance: bool) -> np.ndarray:
    bgr = to_bgr(rgb_in)

    # 1) Page warp (optional)
    if warp:
        quad = _largest_quad_contour(bgr)
        if quad is not None:
            bgr = _warp_from_quad(bgr, quad)

    # 2) Auto deskew (optional)
    if deskew:
        ang = _auto_skew_angle(bgr)
        # Clamp to a sensible bound (same flavor as your previous constraint)
        ang = max(-30.0, min(30.0, ang))
        if abs(ang) >= 0.2:
            bgr = _rotate_bound(bgr, ang)

    # 3) Manual rotation (applied after auto)
    if manual_rot_deg != 0:
        bgr = _rotate_bound(bgr, float(manual_rot_deg))

    # 4) Resize to target width (keep aspect)
    h, w = bgr.shape[:2]
    if w != target_width:
        scale = target_width / float(w)
        new_h = max(300, int(round(h * scale)))
        bgr = cv2.resize(bgr, (target_width, new_h), interpolation=cv2.INTER_CUBIC)

    # 5) Mild enhancement for camera photos
    if enhance:
        # light denoise
        bgr = cv2.fastNlMeansDenoisingColored(bgr, None, 3, 3, 7, 21)
        # contrast via LAB/CLAHE on L channel
...
# ============================================================
# Existing helpers (bbox letterbox, mapping, trace prep)
# ============================================================
...
# ============================================================
# Models (BBox + three signal variants: vanilla / DyT / TCN)
# ============================================================
...
def _infer_from_state_training_style(state: Dict[str, torch.Tensor]):
    if "proj.weight" not in state:
        raise RuntimeError("Checkpoint missing 'proj.weight' — not bbox-style.")
    w = state["proj.weight"]
    dim   = int(w.shape[0])
    patch = int(w.shape[2])
    out_dim = None
...
def load_bbox_head_training_style(ckpt_path: str, img_size=256, in_ch=3, heads=8, device="cpu"):
    ...
    if "pos" in state and tuple(state["pos"].shape) != tuple(model.pos.shape):
        with torch.no_grad():
            model.pos = nn.Parameter(torch.zeros_like(model.pos))
...
class _TCNBlock(nn.Module):
    def __init__(self, ch, kernel=7, dilation=1, dropout=0.0):
        ...
        self.drop = nn.Dropout(dropout)
    def forward(self, x):
        h = self.dw(x); h = self.pw(h); h = self.norm(h); h = self.act(h); h = self.drop(h)
        return x + h

class ColumnSignalCNN(nn.Module):
    def __init__(self, H_ref=128, d_model=256, depth=8, kernel=7, dropout=0.05, dilations=(1,2,4,8)):
        super().__init__()
        self.embed_col = nn.Linear(H_ref, d_model, bias=False)
        self.blocks = nn.ModuleList([
            _TCNBlock(d_model, kernel=kernel, dilation=dilations[i % len(dilations)], dropout=dropout)
...
def load_models_cached(bbox_ckpt: str, sig_ckpt: str, model_type: str, H_ref: int, max_len: int, img_size_bbox: int):
    ...
    if os.path.isfile(sig_ckpt):
        sd = torch.load(sig_ckpt, map_location=device, weights_only=True)
        ...
        if isinstance(w0, torch.Tensor):
            if w0.ndim == 3:
                ...
                detected_arch = "dyt"
        if detected_arch is None and "pos" in sd:
    ...
    elif effective_arch == "tcn":
        signal_model = ColumnSignalCNN(H_ref=H_ref, d_model=256, depth=8).to(device)
    else:
    ...
    if sd is not None:
        ...
        for k, v in list(pruned.items()):
            if k in cur and tuple(cur[k].shape) != tuple(v.shape):
                pruned.pop(k); missing.append(k)
        signal_model.load_state_dict(pruned, strict=False)
        if missing:
            st.warning(f"Some signal weights were skipped due to shape mismatch: {', '.join(missing[:6])}{' …' if len(missing)>6 else ''}")
        if detected_arch and detected_arch != model_type:
            st.info(f"Auto-detected signal arch: **{detected_arch}** (overrode sidebar selection '{model_type}').")

    bbox_head.eval(); signal_model.eval()
    return device, bbox_head, signal_model, out_dim

# ============================================================
# Inference core (unchanged from your pipeline)
# ============================================================
    ...
    if json_ann is not None:
        ...
        for i, meta in enumerate(metas):
            ...
            if e is not None:
                ...
                if pts_all:
                    local = _clip_points_to_box(pts_all, (x1,y1,x2,y2))
...
# ============================================================
# UI — Input: Upload PNG OR Use Phone Camera
# ============================================================
src_choice = st.radio("Choose input source", ["Upload PNG", "Use phone camera"], horizontal=True)

colL, colR = st.columns([1,1])
with colL:
    if src_choice == "Upload PNG":
        up_png = st.file_uploader("Drop ECG PNG", type=["png"], accept_multiple_files=False)
        up_json= st.file_uploader("(Optional) Matching JSON", type=["json"], accept_multiple_files=False)
        rgb_img = None
        ann = None
        if up_png is not None:
            try:
                rgb_img = read_image_from_bytes(up_png.read())
            except Exception as e:
                st.error(f"Failed to read PNG: {e}")
        ...
        cam_img = st.camera_input("Take a photo of the ECG (landscape recommended)")
        ann = None
        rgb_img = None
        if cam_img is not None:
            try:
                rgb_raw = read_image_from_bytes(cam_img.getvalue())
                rgb_img = standardize_camera_rgb(
                    rgb_raw, warp=DO_WARP, deskew=DO_DESKEW,
                    manual_rot_deg=MANUAL_ROT, target_width=TARGET_WIDTH,
                    enhance=CONTRAST
                )
            except Exception as e:
...
with colR:
    if src_choice == "Use phone camera":
        st.info("Preview after standardization (warp/deskew/resize).")
    else:
        st.info("Preview of uploaded PNG.")
    if 'rgb_img' in locals() and rgb_img is not None:
        st.image(rgb_img, channels="RGB", caption="Input used for inference")
...
# ============================================================
if run_btn:
    if 'rgb_img' not in locals() or rgb_img is None:
        st.error("No image available. Capture or upload first.")
        st.stop()
    ...
    with st.spinner("Running model…"):
        try:
            out = run_inference(rgb_img, ann, cfg)
        except Exception as e:
            st.exception(e)
            st.stop()
    ...
    with tab1:
        ...
        _, buf = cv2.imencode('.png', to_bgr(out["pred_overlay"]))
    ...
    with tab2:
        if out["overlay_both"] is None:
            ...
            _, buf2 = cv2.imencode('.png', to_bgr(out["overlay_both"]))
