...
with st.sidebar:
    ...
    # --- Scan-app style preprocessing toggle ---
    APPLY_SCAN_PREPROC = st.checkbox(
        "Apply 'scan app'-style B/W preprocessing",
        value=True,
        help="Convert cropped ECG to a high-contrast black-and-white scan before inference.",
    )
...
def simulate_scanner_style(rgb: np.ndarray) -> np.ndarray:
    """
    Make lighting more even while keeping the original ECG look.
    - No hard black/white.
    - Keep pink grid and black trace.
    - Just flatten shadows/highlights + mild contrast.
    ...
    # 1. Work in LAB space (L = lightness)
    lab = cv2.cvtColor(img, cv2.COLOR_RGB2LAB)
    L, a, b = cv2.split(lab)

    # 2. Estimate slow-varying illumination with a big blur
    #    (captures the “center bright” vs “edge dark” effect)
    illum = cv2.GaussianBlur(L, (0, 0), sigmaX=40, sigmaY=40)

    # 3. Flatten lighting: boost details, reduce large-scale shading
    #    L_flat = 1.5 * L - 0.5 * illum
    L_flat = cv2.addWeighted(L, 1.5, illum, -0.5, 0)
    L_flat = np.clip(L_flat, 0, 255).astype(np.uint8)

    # 4. Mild local contrast enhancement (not aggressive)
    clahe = cv2.createCLAHE(clipLimit=1.8, tileGridSize=(16, 16))
    L_enh = clahe.apply(L_flat)

    # 5. Recompose and go back to RGB
    lab_enh = cv2.merge((L_enh, a, b))
    out = cv2.cvtColor(lab_enh, cv2.COLOR_LAB2RGB)
...
if run_btn:
    ...
    # Apply scan-app style B/W preprocessing (if enabled)
    if APPLY_SCAN_PREPROC:
        scanned_rgb = simulate_scanner_style(resized_rgb)
    else:
        scanned_rgb = resized_rgb

    # Optional: show what is actually fed to the model
    st.image(
        scanned_rgb,
        channels="RGB",
        caption="Preprocessed 'scan-like' ECG used for inference",
    )
    ...
    with st.spinner("Running model…"):
        try:
            out = run_inference(scanned_rgb, ann, cfg)
        except Exception as e:
            st.exception(e)
            st.stop()
