...
import os, io, json, math, tempfile, re
...
with st.sidebar:
    ...
    MODEL_TYPE = st.selectbox(
        "Signal architecture",
        options=["vanilla", "dyt", "tcn"],   # includes your new TCN
        index=1,
        help="ColumnSignalTransformer (vanilla), DyT, or TCN."
    )
    ...
    # --- Auto-rotation (deskew) controls ---
    rotate_enable = st.toggle("Auto-rotate (deskew)", value=True, help="Estimate small tilt with HoughLinesP and rotate before models run.")
    angle_limit   = st.slider("Max abs angle (deg)", 1, 15, 8)
    min_lines     = st.slider("Min parallel lines", 3, 50, 10)
...
# ------------------------------
# Rotation helpers (deskew via HoughLinesP)
# ------------------------------

def _detect_deskew_angle_hough(rgb: np.ndarray, angle_limit_deg: int = 8, min_lines: int = 10) -> float:
    """Return median angle (deg) of near-horizontal lines. Positive = CCW tilt in original."""
    try:
    ...
    lines = cv2.HoughLinesP(edges, rho=1, theta=np.pi/180, threshold=100,
                            minLineLength=max(20, int(0.25*W)),
                            maxLineGap=max(10, int(0.02*W)))
    if lines is None:
        return 0.0
    angles = []
    for x1, y1, x2, y2 in lines.reshape(-1, 4):
        ...
        if abs(ang) <= float(angle_limit_deg):
            angles.append(float(ang))
    if len(angles) < int(min_lines):
        return 0.0
    return float(np.median(angles))

def _rotate_image_and_ann(rgb: np.ndarray, ann: Dict[str, Any] | None, angle_deg: float):
    """Rotate image by -angle (deskew). Also rotate annotation points/bboxes when present."""
    if abs(float(angle_deg)) < 1e-3:
        return rgb, ann, np.eye(3, dtype=np.float32)

    H, W = rgb.shape[:2]
    center = (W / 2.0, H / 2.0)
    M2 = cv2.getRotationMatrix2D(center, -float(angle_deg), 1.0)
    rot = cv2.warpAffine(rgb, M2, (W, H), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REPLICATE)
    M = np.vstack([M2, np.array([0, 0, 1], dtype=np.float32)])  # 3x3

    if ann is None or not isinstance(ann, dict):
        return rot, ann, M

    ann2 = dict(ann)
    if 'leads' in ann2 and isinstance(ann2['leads'], list):
        new_leads = []
        for ld in ann2['leads']:
            ld2 = dict(ld)
            if isinstance(ld2.get('plotted_pixels'), list) and ld2['plotted_pixels']:
                pts = np.array([[p[1], p[0], 1.0] for p in ld2['plotted_pixels']], dtype=np.float32).T
                tp  = (M @ pts).T
                ld2['plotted_pixels'] = [[float(y), float(x)] for x, y, _ in tp]
            if isinstance(ld2.get('lead_bounding_box'), dict):
                bb  = ld2['lead_bounding_box']
                pts = np.array([[bb[str(i)][1], bb[str(i)][0], 1.0] for i in range(4)], dtype=np.float32).T
                tp  = (M @ pts).T
                ld2['lead_bounding_box'] = {str(i): [float(tp[i][1]), float(tp[i][0])] for i in range(4)}
            new_leads.append(ld2)
        ann2['leads'] = new_leads
    return rot, ann2, M
...
# ============================================================
# ColumnSignalCNN (TCN-style) — drop-in alternative
# ============================================================
...
class ColumnSignalCNN(nn.Module):
    ...
    def __init__(self, H_ref=128, d_model=256, depth=8, kernel=7, dropout=0.05, dilations=(1,2,4,8)):
        ...
        nn.init.kaiming_normal_((self.embed_col).weight, nonlinearity="relu")
...
def load_models_cached(bbox_ckpt: str, sig_ckpt: str, model_type: str, H_ref: int, max_len: int, img_size_bbox: int):
    ...
    if os.path.isfile(sig_ckpt):
        ...
        if isinstance(w0, torch.Tensor):
            if w0.ndim == 3:
                detected_arch = "tcn"      # Conv1d head
            elif w0.ndim == 2 and w0.shape[0] == 1:
                detected_arch = "dyt"      # Linear head
        else:
            ln0 = sd.get("head.0.weight", None)
            if isinstance(ln0, torch.Tensor) and ln0.ndim == 1:
                detected_arch = "vanilla"

    effective_arch = detected_arch or model_type

    # ---- Build the selected/effective architecture ----
    if effective_arch == "vanilla":
        ...
        signal_model = ColumnSignalCNN(H_ref=H_ref, d_model=256, depth=8, kernel=7, dropout=0.05).to(device)

    # ---- Safely load: drop incompatible keys (shape-mismatch) ----
    if sd is not None:
        pruned = sd.copy()
        ...
        for k, v in list(pruned.items()):
            if k in cur and tuple(cur[k].shape) != tuple(v.shape):
                pruned.pop(k); skipped.append(k)
        signal_model.load_state_dict(pruned, strict=False)
        if skipped:
            st.warning(f"Skipped {len(skipped)} incompatible keys while loading {effective_arch} checkpoint.")
        if detected_arch and detected_arch != model_type:
            st.info(f"Auto-detected architecture from checkpoint: **{detected_arch}** (overrode sidebar '{model_type}').")
...
def run_inference(rgb: np.ndarray, json_ann: Dict[str, Any] | None, cfg: Dict[str, Any]):
    ...
    # --- optional deskew before any model ---
    if cfg.get("ROTATE_ENABLE", True):
        _ang = _detect_deskew_angle_hough(
            rgb,
            angle_limit_deg=int(cfg.get("ROT_ANGLE_LIMIT", 8)),
            min_lines=int(cfg.get("ROT_MIN_LINES", 10))
        )
        rgb, json_ann, _ = _rotate_image_and_ann(rgb, json_ann, _ang)
        if abs(_ang) > 1e-3:
            st.info(f"Auto-rotation applied: {_ang:.2f}°")
    ...
    if json_ann is not None:
        ...
        # Averages (simple and width-weighted)
...
if run_btn and up_png is not None:
    ...
    cfg = dict(
        ...
        ROTATE_ENABLE=rotate_enable, ROT_ANGLE_LIMIT=angle_limit, ROT_MIN_LINES=min_lines,
    )
