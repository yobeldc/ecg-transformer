...
with st.sidebar:
    ...
    # --- Architecture selector (now includes TCN) ---
    MODEL_TYPE= st.selectbox(
        "Signal architecture",
        options=["vanilla", "dyt", "tcn"],
        index=1,
        help="Matches your ColumnSignalTransformer (vanilla), DyT variant, or TCN."
    )
...
def load_models_cached(bbox_ckpt: str, sig_ckpt: str, model_type: str, H_ref: int, max_len: int, img_size_bbox: int):
    ...
    if os.path.isfile(sig_ckpt):
        ...
        if isinstance(w0, torch.Tensor):
            if w0.ndim == 3:            # Conv1d head → TCN
                detected_arch = "tcn"
            elif w0.ndim == 2:          # Linear head → DyT / vanilla
                detected_arch = "dyt"   # prefer DyT for linear-first heads
        # fallbacks
    ...
    # Safely load: drop incompatible keys (e.g., LayerNorm vs Linear)
    if sd is not None:
        pruned = sd.copy()
        missing = []
