...
def simulate_scanner_style(rgb: np.ndarray) -> np.ndarray:
    ...
    - Flatten shadows/highlights + mild contrast enhancement.
    """
    ...
    # Work in LAB: L = lightness, a/b = color
    lab = cv2.cvtColor(img, cv2.COLOR_RGB2LAB)
    L, a, b = cv2.split(lab)

    # Estimate slow-varying illumination field with big blur
    illum = cv2.GaussianBlur(L, (0, 0), sigmaX=40, sigmaY=40)

    # Flatten lighting: boost L, subtract some illumination
    ...
    # Recompose to LAB and back to RGB
